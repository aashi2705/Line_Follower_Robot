...
Overview:
This project implements a basic line-following robot using Arduino. The robot follows a line on the ground by using two IR sensors and controlling the direction and speed of two DC motors. Based on the sensor readings, the robot moves straight, turns left, turns right, or stops to stay aligned with the track.
...
2- rotateMotor(): Controls motor direction and speed for both motors based on the values passed to it. Positive speed moves a motor forward, negative speed moves it backward, and zero stops it.
